/*
 * This program computes the equivalent binary value of a decimal input
 *
 */
 ...
 class Converter {
 			...
 			if(decimal < 2){
				binary = binary + decimal;
		 		...
		 		while(0-1 < powersOfTwo){
		 			...
		 			// testing if powersOfTwo is a multiple of 4 (modulo isn't implemented >_< )
 	...
 	def addBeginningZeros(numBits : Int) : String = {
 		...
 		result = "";
 		count = numBits-(numBits/4)*4; // number of bits in first bloc of binary value
 		
 		if(0 < count){
 			count = 4 - count; // number of missing 0 at the beginning of first bloc of bits
 		}
