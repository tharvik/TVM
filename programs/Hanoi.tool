...
class HanoiPresenter {
	var towerA : Tower;
	 ...
	 def play() : Boolean = {
		...
		if(! this.mvRing(N, towerA, towerC, towerB)) {
			println("Error !!!");
		 }
		println("Done in " + steps + " steps.");
		 return true;
	 }
	def mvRing2(a : Tower, b : Tower) : Boolean = {
		var v : Boolean;
		 v = b.push(a.pop());
		 steps = steps + 1;
		 if(! this.printAll()) {
	...
	def mvRing(nb : Int, a : Tower, b : Tower, t : Tower) : Boolean = {
		var e : Boolean;
		 if(nb == 1) {
			e = this.mvRing2(a, b);
		 }
