	...
	var top : Tree;
	 def getNew() : Expr = {
		var left : Tree;
		 ...
		 top = new Fork().init(left, right, 1);
		 left = top;
		 right = new Leaf().init(2);
		 top = new Fork().init(left, right, 3);
		 right = new Leaf().init(34);
		 left = new Leaf().init(1367);
		 top = new Fork().init(new Fork().init(left, right, 4), top, 2);
		 return this;
	 }
	def printAll() : String = {
...
class Fork extends Tree {
	...
	def inPrint() : String = {
		return "(" + l.inPrint() + " " + this.decode(val) + " " + r.inPrint() + STR())(134:81);
	 }
}
